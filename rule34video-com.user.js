...
(function () {
  ...
  // La marca («sin «pmv»») va en la esquina libre de la miniatura: abajo a la derecha (la web pone
  // la duración abajo a la izquierda y el «HD» arriba a la derecha). Dentro de la línea del título no
  // se puede: el título lleva `overflow:hidden` con puntos suspensivos y se la comería.
  function holderFor(card) {
    return card.querySelector(".wrap_image") || card.querySelector(".img") || card.querySelector(".thumb_title") || card;
  }

  function markFor(card) {
    var holder = holderFor(card);
    var el = holder.querySelector(":scope > ." + MARK);
    if (!el) {
