...
(function () {
  ...
  function build() {
    ...
    var ORDER = ["gallery", "downloads", "score", "video", "tags", "enhancer", "changelog"];
...
(function () {
  ...
  // ---- Añadir etiquetas a la búsqueda sin salir de la página ----
  // Cada etiqueta del panel lateral gana dos botones: «+» la añade a la caja de búsqueda del sitio y
  // «−» la excluye. Los botones solo escriben en la caja (y marcan la etiqueta en verde/rojo); la
  // búsqueda no se lanza hasta que se pulsa «Search», así se pueden ir juntando varias etiquetas
  // viendo la consulta antes de buscar.

  var tokHinted = false;

  function searchField() {
    return (
      document.querySelector(".tag-search form input[name='tags']") ||
      document.querySelector(".tag-search input[name='tags']") ||
      document.querySelector("form input[name='tags']") ||
      null
    );
  }
  ...
  function flashSearchField() {
    var f = searchField();
    if (!f) return;
    f.classList.remove("r34g-flash");
    void f.offsetWidth;
    f.classList.add("r34g-flash");
    setTimeout(function () {
      f.classList.remove("r34g-flash");
    }, 1000);
  }
  ...
  // + = incluir esa etiqueta (si ya estaba, la quita) · − = excluirla (si ya estaba excluida, la quita)
  function tokenToggle(name, exclude) {
    name = String(name || "").trim();
    if (!name) return false;
    if (!searchField()) {
      util.toast("En esta página no hay caja de búsqueda");
      return false;
    }
    var want = (exclude ? "-" : "") + name;
    var tokens = searchTokens();
    var had = tokens.some(function (t) {
      return t === want;
    });
    var kept = tokens.filter(function (t) {
      return t.replace(/^-/, "") !== name;
    });
    if (!had) kept.push(want);
    setSearchTokens(kept);
    syncTagStates();
    if (had) {
      util.toast("«" + name + "» fuera de la búsqueda", 1600);
      return true;
    }
    var msg = (exclude ? "Excluida: " : "Añadida: ") + "«" + name + "»";
    if (!tokHinted) {
      tokHinted = true;
      msg += " — verás la consulta en la caja de búsqueda; pulsa Search para aplicarla";
      util.toast(msg, 4200);
    } else {
      util.toast(msg, 1800);
    }
  ...
  function tagNameOf(li) {
    if (li.dataset.r34gTag) return li.dataset.r34gTag;
    var a = li.querySelector("a[href*='tags=']");
    var m = a ? /[?&]tags=([^&]+)/.exec(a.getAttribute("href") || "") : null;
    var name = "";
    if (m) {
      try {
    ...
    if (name) li.dataset.r34gTag = name;
    return name;
  }
  ...
  function syncTagStates() {
    ...
    tokens.forEach(function (t) {
      if (t.charAt(0) === "-") exc[t.slice(1)] = 1;
      else inc[t] = 1;
    });
    var rows = document.querySelectorAll("#tag-sidebar li, .sidebar ul li");
    Array.prototype.forEach.call(rows, function (li) {
      var name = tagNameOf(li);
      if (!name) return;
      var isIn = !!inc[name];
      var isOut = !!exc[name];
      li.classList.toggle("r34g-tag-in", isIn);
      li.classList.toggle("r34g-tag-out", isOut);
      var add = li.querySelector(".r34g-tag-add");
      var sub = li.querySelector(".r34g-tag-sub");
      if (add) add.classList.toggle("r34g-on", isIn);
      if (sub) sub.classList.toggle("r34g-on", isOut);
    });
  }
  ...
  function buildTagActions() {
    ...
    Array.prototype.forEach.call(list.querySelectorAll("li"), function (li) {
      if (li.dataset.r34gActs === "1") return;
      if (li.querySelector("h6")) return;
      var name = tagNameOf(li);
      if (!name) return;
      var old = tagActionLinks(li);
      if (!old.add && !old.sub && !li.querySelector("a[href*='tags=']")) return;
    ...
    var f = searchField();
    if (f && f.dataset.r34gTok !== "1") {
      f.dataset.r34gTok = "1";
      f.addEventListener("input", syncTagStates);
      f.addEventListener("change", syncTagStates);
    }
    syncTagStates();
    return n;
  }
  ...
  // ---- Barra del analizador en la propia página (fuera de Ajustes) ----
  // Va justo encima de la barra «Miniaturas», encima de la cuadrícula. El bloque del analizador que
  // ya existe en la página (con sus estilos) se muda dentro de esta barra, para aprovechar su panel
  // de resultados en vez de duplicar la interfaz. En las páginas de post no se pone: allí ya está el
  // bloque «Etiquetas por personaje» junto al archivo.

  var TAGS_BAR_ID = "r34g-tags-bar";
  ...
  function createTagsBar() {
    var bar = util.el("div", "r34g-tags-bar");
    bar.id = TAGS_BAR_ID;
    var title = util.el("button", "r34g-tags-bar-title");
    title.type = "button";
    title.title = "Analiza las etiquetas del panel lateral y las reparte por personaje";
    title.appendChild(util.icon("tag"));
    title.appendChild(util.el("span", null, "An\u00e1lisis de etiquetas"));
    var caret = util.el("span", "r34g-tags-bar-caret", "\u25b8");
    ...
    title.addEventListener("click", function () {
      ...
      panel.hidden = !show;
      bar.classList.toggle("r34g-tags-bar-on", show);
    });
    bar.appendChild(title);
    var note = util.el("span", "r34g-note", "Reparte las etiquetas de esta p\u00e1gina entre sus personajes");
    note.title = "Usa el panel lateral de etiquetas: lo mismo que el bloque \u00abEtiquetas por personaje\u00bb de un post";
    bar.appendChild(note);
    var run = util.el("button", "r34g-mini", "Analizar");
    run.type = "button";
  ...
  function syncPostToolbar() {
    ...
    if (!bar) {
      bar = createTagsBar();
      var gridBar = document.getElementById(GRID_BAR_ID);
      var anchor = gridBar && gridBar.parentNode === list.parentNode ? gridBar : list;
      anchor.parentNode.insertBefore(bar, anchor);
    }
    if (panel.parentNode !== bar) {
      panel.hidden = true;
      var launch = panel.querySelector(".r34g-tp-launch");
      if (launch) launch.hidden = true;
      bar.appendChild(panel);
    }
    var gridBarNow = document.getElementById(GRID_BAR_ID);
    if (gridBarNow && gridBarNow.parentNode === bar.parentNode && bar.nextElementSibling !== gridBarNow) {
      gridBarNow.parentNode.insertBefore(bar, gridBarNow);
    }
    return true;
  }
  ...
  R.tagBuilder = {
    build: buildTagActions,
    sync: syncTagStates,
    tokens: searchTokens,
    add: function (name) {
      return tokenToggle(name, false);
    },
    exclude: function (name) {
      return tokenToggle(name, true);
    },
    clear: function () {
      return setSearchTokens([]);
    }
  };

  R.analysisBar = {
    sync: syncPostToolbar,
    run: runAnalysis
  };
  ...
  R.onReady(function () {
    ...
    buildTagActions();
    syncPostToolbar();
    // El panel lateral y el bloque del analizador pueden llegar tarde (los pinta rule34 / otro módulo):
    // reintentamos unas cuantas veces.
    var tagTries = 0;
    var tagPoll = setInterval(function () {
      tagTries++;
      var acts = buildTagActions();
      var bar = syncPostToolbar();
      if ((acts && bar) || tagTries > 20) clearInterval(tagPoll);
    }, 700);
...
(function () {
  ...
  function promptText(result) {
    var spaces = R.settings.tPromptFmt === "espacios";
    var sep = spaces ? " " : ", ";
    var pre = String(R.settings.tPromptPre || "").trim();
    var suf = String(R.settings.tPromptSuf || "").trim();
    var body = promptTags(result).join(sep);
    var bits = [];
    if (pre) bits.push(pre);
    if (body) bits.push(body);
    if (suf) bits.push(suf);
    var text = bits.join(sep).trim();
    return spaces ? text.replace(/\s+/g, " ") : text;
  }
  ...
  function promptBox(host, result) {
    ...
    function modeBtn(label, value, title) {
      var b = util.el("button", "r34g-prompt-opt", label);
      b.type = "button";
      b.title = title;
    ...
    ta.rows = 4;
    ta.spellcheck = false;
    ta.title = "Puedes retocarlo aqu\u00ed antes de copiarlo";
  ...
  function renderInto(host, result) {
    ...
    var promptBtn = util.el("button", "r34g-mini", "Copiar prompt");
    promptBtn.type = "button";
    promptBtn.title = "Copia el prompt listo para pegar en otra app (imagen o v\u00eddeo)";
    ...
    host.appendChild(promptBox(host, result));
  ...
  function buildPanel(panel) {
    panel.appendChild(
      util.el(
        "p",
        "r34g-hint",
        "Esta sección reparte las etiquetas de un post entre sus personajes, detecta etiquetas redundantes y te deja ajustar el resultado antes de copiarlo, buscarlo en rule34 o sacarle un prompt listo para pegar en cualquier app de imagen o de vídeo (esta herramienta no genera imágenes). Los botones para analizar están fuera de Ajustes: en un post, en el bloque «Etiquetas por personaje»; y en las listas de búsqueda, en la barra «Análisis de etiquetas» que hay encima de la barra de miniaturas."
      )
    );
    ...
    ui.seg({
      ...
      key: "tPromptFmt",
      title: "Formato",
      note: "\u00abComas\u00bb encaja en casi cualquier app; \u00abetiquetas con espacios\u00bb imita la lista de los modelos tipo danbooru (NovelAI, Pony).",
      options: [
        { label: "Comas", value: "comas" },
        { label: "Etiquetas con espacios", value: "espacios" }
      ]
    });
    ui.toggle({
      section: promptSec,
    ...
    ui.toggle({
      section: promptSec,
      key: "tPromptMeta",
      title: "Incluir el medio y los metadatos",
      note: "video, webm, sound, watermark\u2026 No describen la escena, as\u00ed que se quedan fuera por defecto."
    });
    ui.text({
      section: promptSec,
      ...
      set: function (v) {
        R.set("tPromptPre", String(v));
      },
      event: "change"
    });
    ui.text({
      section: promptSec,
      title: "Final del prompt",
      note: "Opcional. Se pega despu\u00e9s de las etiquetas.",
      placeholder: "detailed, 4k",
      get: function () {
        return R.settings.tPromptSuf;
      },
      set: function (v) {
        R.set("tPromptSuf", String(v));
      },
      event: "change"
    });
  ...
  R.onChange(function (key) {
    if (String(key || "").indexOf("tPrompt") !== 0) return;
    var host = document.getElementById("r34g-tp-page");
    if (host && host.dataset.r34gResult && R.lastAnalysis) paintPrompt(host, R.lastAnalysis);
  });
...
(function () {
  ...
  // ── Recomendador ────────────────────────────────────────────────────────────────────────────────
  // Cuando la búsqueda es un personaje (o cualquier etiqueta) y su galería está centrada en él, este
  // módulo lee lo que ya trae la propia página — la puntuación que enseña rule34 y las etiquetas de
  // cada miniatura, que vienen en el title de la imagen — y le pone una nota a cada post para decir
  // cuáles merecen la pena. Todo local: no gasta API, no necesita clave y funciona para todo el mundo.
  //
  // Sobre las "visualizaciones": rule34.xxx no publica cuánta gente ha visto cada post, así que se
  // usa la puntuación (los votos, que es lo que enseña la web) y, para saber cuánto tira el personaje,
  // el número de posts que tiene su etiqueta en todo el sitio (de ahí el "+N posts en el sitio").

  var BAR_ID = "r34g-score-bar";
  var PANEL_ID = "r34g-score-panel";
  var THUMB_SEL = ".image-list .thumb";

  // Etiquetas que no cuentan al comparar dos posts para ver si son la misma escena en distintas
  // versiones (formato, calidad, duración, idioma...). Lo que queda es el "contenido" de la escena.
  var VOLATIL = [
    "video","animated","mp4","webm","gif","animated_gif","sound","no_sound","has_sound","music",
    "sound_effects","voice_acted","audible","audible_ejaculation","3d","2d","video_games","lowres",
    "low_res","pixelated","bad_quality","absurd_res","highres","hi_res","hd","uncensored","censored",
    "english_text","text","commentary","watermark","tagme","self_upload","longer_version_available",
    "shorter_version_available","loop","perfect_loop","shorter_than_10_seconds","shorter_than_30_seconds",
    "longer_than_10_seconds","longer_than_30_seconds","short_playtime","long_playtime","1_second_video",
    "<1_second_video","16:9","9:16","4:3","3:4","1:1","4:5","5:4","vertical_video","horizontal_video",
    "extended","version"
  ];

  var LOW = { lowres: 1, low_res: 1, pixelated: 1, bad_quality: 1 };
  var HIGH = { absurd_res: 1, highres: 1, hi_res: 1, hd: 1 };
  var SOUND = { sound: 1, voice_acted: 1, audible: 1, audio: 1, has_sound: 1, sound_effects: 1 };
  var SHORT = {
    shorter_than_10_seconds: 1,
    "1_second_video": 1,
    "<1_second_video": 1,
    short_playtime: 1,
    very_short: 1
  };
  var LONG = { longer_than_10_seconds: 1, longer_than_30_seconds: 1, long_playtime: 1 };

  var state = { posts: [], rec: [], q: null, char: null, marca: "" };
  ...
  // El title de la miniatura trae "tag1 tag2 ... tagN score:123 rating:explicit".
  function tagsOf(text) {
    return unescapeHtml(text)
      .replace(/\b(score|rating|user|id|status|size|source|tags|date|parent|pool)\s*:\s*\S+/gi, " ")
      .replace(/\s+/g, " ")
      .trim()
      .split(" ")
      .map(normTag)
  ...
  function scoreOf(thumb, text) {
    var m = /score\s*:\s*(-?\d+)/i.exec(text);
    if (m) return Number(m[1]);
    var si = thumb.querySelector(".score-info");
    if (!si) return null;
    var n = /-?\d+/.exec(String(si.textContent || ""));
    return n ? Number(n[0]) : null;
  }

  function postFrom(thumb) {
    var img = thumb.querySelector("img.preview") || thumb.querySelector("img");
    var a = thumb.querySelector("a[href*='id=']") || thumb.querySelector("a");
    var href = (a && a.getAttribute("href")) || "";
    var id = util.postIdFromHref ? util.postIdFromHref(href) : "";
    if (!id && thumb.id) {
      var m = /^s(\d+)$/.exec(thumb.id);
      if (m) id = m[1];
    }
    var text = (img && (img.getAttribute("title") || img.getAttribute("alt"))) || "";
    var tags = tagsOf(text);
    var set = {};
    tags.forEach(function (t) {
      set[t] = 1;
    });
    ...
    var p = {
      ...
      tags: tags,
      set: set,
      score: scoreOf(thumb, text),
      rating: ((/rating\s*:\s*([a-z]+)/i.exec(text) || [])[1] || "").toLowerCase(),
      video: /(^|\s)webm-thumb(\s|$)/.test(cls) || !!set.video || !!set.animated || !!set.mp4,
      nota: 0,
      why: [],
      dup: "",
      rec: 0
  ...
  function readPosts(scope) {
    var root = scope && scope.querySelectorAll ? scope : document;
    var out = [];
    Array.prototype.forEach.call(root.querySelectorAll(THUMB_SEL), function (th) {
      var p = postFrom(th);
      if (p.id) out.push(p);
    });
    return out;
  ...
  // Etiquetas que se están buscando (la primera es la "enfocada": el personaje, la serie...).
  function queryInfo() {
    var raw = "";
    if (typeof R.downloads.tags === "function") raw = String(R.downloads.tags() || "");
    else {
      try {
        raw = new URLSearchParams(location.search).get("tags") || "";
      } catch (e) {
        raw = "";
      }
    }
    ...
    String(raw)
      ...
      .forEach(function (w) {
        if (!w || w === "all") return;
        var malo = w.charAt(0) === "-";
        var t = normTag(malo ? w.slice(1) : w);
        if (!t || /^(rating|score|user|id|status|size|source|parent|pool):/.test(t)) return;
        var list = malo ? neg : pos;
        if (list.indexOf(t) === -1) list.push(t);
      });
    return { pos: pos, neg: neg, raw: raw, tag: pos[0] || "" };
  }

  // Del panel lateral: cuántos posts tiene la etiqueta en todo el sitio (su "tirón") y de qué tipo es.
  function sidebarTag(name) {
    if (!name) return null;
    ...
    Array.prototype.forEach.call(side.querySelectorAll("li"), function (li) {
      ...
      var a = li.querySelector("a[href*='tags=']");
      if (!a) return;
      var m = /[?&]tags=([^&]+)/.exec(a.getAttribute("href") || "");
      if (!m) return;
      var t = "";
      try {
        t = normTag(decodeURIComponent(m[1].replace(/\+/g, " ")));
      } catch (e) {
        t = normTag(m[1]);
      }
      if (t !== name) return;
      var c = li.querySelector(".tag-count");
      found = {
        name: name,
        count: util.num(c ? c.textContent : 0),
        type: (/tag-type-([a-z]+)/.exec(li.className || "") || [])[1] || ""
      };
    });
    return found;
  }
  ...
  function coreSet(p) {
    ...
    p.tags.forEach(function (t) {
      if (VOLATIL.indexOf(t) === -1) out[t] = 1;
    });
    return out;
  }
  ...
  function jaccard(a, b) {
    ...
    var union = n + Object.keys(b).length - inter;
    return union ? inter / union : 0;
  }

  // Variantes de la misma escena (mismo contenido, distinto formato o duración).
  function variantGroups(posts) {
    var cores = {};
    posts.forEach(function (p) {
      cores[p.id] = coreSet(p);
    });
    var groups = [];
    var visto = {};
    posts.forEach(function (p) {
      if (visto[p.id]) return;
      visto[p.id] = 1;
      var n = Object.keys(cores[p.id]).length;
      if (n < 6) return;
      var group = [p];
      posts.forEach(function (o) {
        if (o === p || visto[o.id]) return;
        if (Object.keys(cores[o.id]).length < 6) return;
        if (jaccard(cores[p.id], cores[o.id]) >= 0.8) {
          group.push(o);
          visto[o.id] = 1;
        }
      });
  ...
  function has(p, map) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (p.set[keys[i]]) return keys[i];
    }
    return "";
  }

  function p90Of(values) {
    var v = values.slice().sort(function (a, b) {
      return a - b;
    });
  ...
  function pointsOf(p, ctx) {
    var s = 0;
    var why = [];
    var sc = p.score == null ? 0 : p.score;

    if (ctx.maxLog > 0 && sc > 0) {
      s += 45 * (Math.log(1 + sc) / ctx.maxLog);
      why.push(sc >= ctx.p90 ? "de las mejores puntuaciones de la p\u00e1gina (" + util.pretty(sc) + ")" : "puntuaci\u00f3n " + util.pretty(sc));
    } else {
      why.push("sin puntuaci\u00f3n");
    }

    if (ctx.q.pos.length) {
      var falta = ctx.q.pos.filter(function (t) {
        return !p.set[t];
      });
      if (!falta.length) {
        s += 20;
        why.push("lleva " + ctx.q.pos.join(" + ").replace(/_/g, " "));
      } else {
        s -= 25;
        why.push("no lleva " + falta.join(", ").replace(/_/g, " "));
      }
    }
    var fuera = ctx.q.neg.filter(function (t) {
      return p.set[t];
    });
    if (fuera.length) {
      s -= 20;
      why.push("tiene " + fuera.join(", ").replace(/_/g, " ") + " (excluida en la b\u00fasqueda)");
    }

    s += Math.min(8, (p.tags.length / 30) * 8);
    if (p.tags.length < 8) why.push("con pocas etiquetas");

    var low = has(p, LOW);
    if (low) {
      s -= 10;
      why.push("marcada como de baja resoluci\u00f3n");
    }
    if (has(p, HIGH)) {
      s += 5;
      why.push("de alta resoluci\u00f3n");
    }
    if (p.video) {
      s += ctx.video;
      why.push("es v\u00eddeo");
    ...
    if (has(p, LONG)) {
      s += 2;
      why.push("de los largos");
    }
    if (p.set.uncensored) {
      s += 2;
      why.push("sin censura");
    }
    if (ctx.serieEtiqueta && p.set[ctx.serieEtiqueta]) {
      s += 3;
      why.push("de la serie buscada");
    }
    if (p.set.longer_version_available) {
      s -= 10;
      why.push("hay una versi\u00f3n m\u00e1s larga");
    }
    if (ctx.seen && R.seen && R.seen.has && R.seen.has(p.id)) {
      s -= 8;
      why.push("ya visto");
    }
    if (ctx.hist && R.downloads && R.downloads.hist && R.downloads.hist.has && R.downloads.hist.has(p.id)) {
      s -= 10;
      why.push("ya descargado");
    }
    return { s: s, why: why };
  }

  function evaluate(posts, q) {
    if (!posts.length) return [];
    var valores = posts.map(function (p) {
    ...
    var ctx = {
      q: q,
      p90: p90Of(valores),
      maxLog: Math.log(1 + Math.max(1, max)),
      video: Math.max(0, Math.min(10, util.num(R.settings.scVideo))),
      seen: R.settings.scSeen !== false,
      hist: R.settings.scHist !== false,
      serieEtiqueta: ""
    };
    if (q.pos.length > 1) {
      var serie = sidebarTag(q.pos[1]);
      if (serie) ctx.serieEtiqueta = q.pos[1];
    }

    posts.forEach(function (p) {
      var out = pointsOf(p, ctx);
      p.nota = out.s;
      p.why = out.why;
      p.dup = "";
    });

    if (R.settings.scDup !== false) {
      variantGroups(posts).forEach(function (group) {
        var mejor = group[0];
        group.forEach(function (p) {
          if (p.nota > mejor.nota) mejor = p;
        });
        group.forEach(function (p) {
          if (p === mejor) return;
          p.nota -= 18;
          p.dup = mejor.id;
          p.why.push("variante de #" + mejor.id);
        });
      });
    }

    posts.forEach(function (p) {
      p.nota = Math.max(0, Math.min(100, Math.round(p.nota)));
    });
  ...
  function pickRecommended(posts) {
    var modo = R.settings.scMode === "min" ? "min" : "top";
    var n = util.num(R.settings.scTop) || 12;
    var min = util.num(R.settings.scMin);
    if (isNaN(min) || min <= 0) min = 60;
    var orden = posts.slice().sort(function (a, b) {
      return b.nota - a.nota;
    });
    var out;
    if (modo === "min") {
      out = orden.filter(function (p) {
        return p.nota >= min;
      });
    } else {
      out = orden.slice(0, n).filter(function (p) {
        return p.nota >= 30;
      });
    }
    return out;
  ...
  function tier(n) {
    if (n >= 80) return "r34g-score-hi";
    if (n >= 60) return "r34g-score-mid";
    return "r34g-score-lo";
  }

  function markThumbs() {
    state.posts.forEach(function (p) {
      var old = p.thumb.querySelector(".r34g-score-badge");
      if (old) old.remove();
      p.thumb.classList.remove("r34g-score-rec", "r34g-score-dup", "r34g-score-top");
      p.thumb.classList.add("r34g-scored");
      if (!R.settings.scBadges) return;
      if (p.dup) p.thumb.classList.add("r34g-score-dup");
      if (p.rec) {
        p.thumb.classList.add("r34g-score-rec");
        if (p.rec <= 3) p.thumb.classList.add("r34g-score-top");
      }
      var b = util.el("span", "r34g-score-badge " + tier(p.nota), String(p.nota));
      b.title =
        "Recomendador: " + p.nota + "/100\n" +
        p.why.join("\n") +
        (p.dup ? "\n\nVariante de #" + p.dup + " (se queda esa)" : "");
      (p.a || p.thumb).appendChild(b);
    });
  }

  function clearMarks() {
    Array.prototype.forEach.call(document.querySelectorAll(".r34g-score-badge"), function (b) {
      b.remove();
    });
    Array.prototype.forEach.call(document.querySelectorAll(THUMB_SEL), function (th) {
      th.classList.remove("r34g-scored", "r34g-score-rec", "r34g-score-dup", "r34g-score-top", "r34g-score-flash");
    });
    state.posts = [];
    state.rec = [];
    state.marca = "";
    ...
    if (bar) bar.classList.remove("r34g-score-bar-on");
    note("Pulsa «Evaluar» para puntuar esta galer\u00eda.");
  }

  function note(text) {
    var el = document.querySelector("#" + BAR_ID + " > .r34g-score-note");
    if (el) el.textContent = text;
  }

  function summary() {
    var posts = state.posts;
    if (!posts.length) return "No veo miniaturas en esta p\u00e1gina.";
    var notas = posts.map(function (p) {
      return p.nota;
    });
    var min = Math.min.apply(null, notas);
    var max = Math.max.apply(null, notas);
    var bits = [state.rec.length + " de " + posts.length + " recomendados"];
    bits.push(min === max ? "nota " + max : "notas " + min + "\u2013" + max);
    if (state.char && state.char.count) {
      bits.push("\u00ab" + state.char.name.replace(/_/g, " ") + "\u00bb tiene " + util.pretty(state.char.count) + " posts en el sitio");
    }
    return bits.join(" \u00b7 ");
  }
  ...
  function renderPanel() {
    ...
    if (!panel) return;
    panel.innerHTML = "";
    panel.appendChild(util.el("p", "r34g-score-head", summary()));

    var how = util.el("p", "r34g-hint");
    how.textContent =
      "La nota mezcla la puntuaci\u00f3n que ya trae rule34 con lo bien etiquetado que est\u00e1 el post, " +
      "su encaje con lo que buscas, la resoluci\u00f3n y la duraci\u00f3n, y resta por reposts, variantes y ya " +
      "vistos. Pasa el rat\u00f3n por la nota de una miniatura para ver el desglose.";
    panel.appendChild(how);

    if (!state.rec.length) {
      panel.appendChild(util.el("p", "r34g-hint", "Nada llega al m\u00ednimo. Baja el criterio en Ajustes \u2192 Recomendador."));
      return;
    }

    var list = util.el("div", "r34g-score-list");
    state.rec.forEach(function (p, i) {
      var row = util.el("div", "r34g-score-row");
      row.appendChild(util.el("span", "r34g-score-pos", "#" + (i + 1)));
      row.appendChild(util.el("b", "r34g-score-num " + tier(p.nota), String(p.nota)));
      var info = util.el("span", "r34g-score-info");
      var linea1 = util.el("span", "r34g-score-id");
      linea1.appendChild(util.el("b", null, "#" + p.id));
      linea1.appendChild(util.el("span", "r34g-note", p.video ? " v\u00eddeo" : ""));
      info.appendChild(linea1);
      info.appendChild(util.el("span", "r34g-note", p.why.slice(0, 3).join(" \u00b7 ")));
      row.appendChild(info);
      var btn = util.el("button", "r34g-mini", "Descargar");
      btn.type = "button";
      btn.title = "Manda este post a la cola de descargas";
      btn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        one(p, btn);
  ...
  function focusThumb(p) {
    if (!p.thumb || !p.thumb.scrollIntoView) return;
    p.thumb.scrollIntoView({ behavior: "smooth", block: "center" });
    p.thumb.classList.add("r34g-score-flash");
    setTimeout(function () {
      p.thumb.classList.remove("r34g-score-flash");
    }, 1400);
  }
  ...
  function api() {
    return R.downloads && R.downloads.api ? R.downloads.api : null;
  }

  function queueOne(p) {
    var a = api();
    if (!a || !a.downloadPost) {
      util.toast("El m\u00f3dulo de descargas no est\u00e1 disponible");
      return false;
    }
    return !!a.downloadPost(p.id, { single: true });
  }

  function one(p, btn) {
    var a = api();
    if (a && a.ensureDirPermission) a.ensureDirPermission();
    if (queueOne(p)) util.toast("Post #" + p.id + " a la cola");
  }

  function queueRecommended() {
    if (!state.rec.length) {
      util.toast("Pulsa \u00abEvaluar\u00bb primero");
      return;
    }
    var a = api();
    if (!a || !a.downloadPost) {
    ...
    if (a.ensureDirPermission) a.ensureDirPermission();
    var n = 0;
    state.rec.forEach(function (p) {
      if (queueOne(p)) n++;
    });
    util.toast(n ? n + " posts a la cola de descargas" : "No se a\u00f1adi\u00f3 ninguno (revisa el historial)");
  }

  function run(manual) {
    if (!R.settings.scOn && !manual) return null;
    var posts = readPosts();
    if (!posts.length) {
      note("No veo miniaturas en esta p\u00e1gina.");
      return null;
    }
    var q = queryInfo();
    state.posts = evaluate(posts, q);
    state.q = q;
    state.char = sidebarTag(q.tag);
    state.rec = pickRecommended(state.posts);
    var orden = {};
    state.rec.forEach(function (p, i) {
      p.rec = i + 1;
      orden[p.id] = i + 1;
    ...
    state.marca = posts.map(function (p) {
      return p.id;
    }).join(",");
    markThumbs();
    renderPanel();
  ...
  function buildBar() {
    ...
    var title = util.el("button", "r34g-score-title");
    title.type = "button";
    title.title = "Puntúa los posts de esta galería y dice cuáles merecen la pena descargar";
    title.appendChild(util.icon("wand"));
    title.appendChild(util.el("span", null, "Recomendador"));
    var caret = util.el("span", "r34g-score-caret", "\u25b8");
    title.appendChild(caret);
    title.addEventListener("click", function () {
      var panel = document.getElementById(PANEL_ID);
      if (!panel) return;
      panel.hidden = !panel.hidden;
      bar.classList.toggle("r34g-score-bar-on", !panel.hidden);
    });
    bar.appendChild(title);

    bar.appendChild(util.el("span", "r34g-note r34g-score-note", "Sin evaluar."));

    var runBtn = util.el("button", "r34g-mini", "Evaluar");
    runBtn.type = "button";
    runBtn.title = "Puntúa de nuevo los posts que hay en la página";
    runBtn.addEventListener("click", function () {
      run(true);
    });
    bar.appendChild(runBtn);

    var dlBtn = util.el("button", "r34g-mini r34g-score-dl", "Descargar los recomendados");
    dlBtn.type = "button";
    dlBtn.title = "Mete en la cola de descargas los posts recomendados, en orden de nota";
    dlBtn.addEventListener("click", queueRecommended);
    bar.appendChild(dlBtn);

    var clear = util.el("button", "r34g-mini", "Quitar notas");
    clear.type = "button";
    clear.title = "Borra las notas de las miniaturas y deja la página limpia";
    clear.addEventListener("click", clearMarks);
    bar.appendChild(clear);

    var panel = util.el("div");
    panel.id = PANEL_ID;
    panel.hidden = true;
    bar.appendChild(panel);
  ...
  // ¿La galería es de un personaje? Se llama "enfocada" si casi todos los posts llevan la etiqueta
  // buscada; si no, la barra se queda a mano (nada de puntuar 40 posts de un tag que no está).
  function focusShare() {
    if (!state.posts.length || !state.q || !state.q.pos.length) return 0;
    var dentro = state.posts.filter(function (p) {
      return state.q.pos.every(function (t) {
        return p.set[t];
      });
    }).length;
    return dentro / state.posts.length;
  }

  // Pinta la barra y, si se le dice que puede (y el ajuste lo permite), puntúa solo cuando la
  // búsqueda es una etiqueta y la galería está centrada en ella. Si no, deja el aviso y espera.
  function sync(mayRun) {
    if (!R.settings.scOn) {
      clearMarks();
      var oculta = document.getElementById(BAR_ID);
      if (oculta) oculta.hidden = true;
      return;
    }
    var bar = ensureBar();
    if (!bar) return;
    ...
    var marca = posts.map(function (p) {
      return p.id;
    }).join(",");
    var q = queryInfo();
    state.posts = posts;
    state.q = q;
    if (!(mayRun && R.settings.scAuto)) {
      if (marca !== state.marca || !state.rec.length) {
        state.marca = "";
        note("Pulsa «Evaluar» para puntuar " + posts.length + " posts.");
      }
      return;
    }
    if (q.pos.length !== 1) {
      note("Esta búsqueda no es una sola etiqueta: pulsa «Evaluar» cuando quieras.");
      return;
    }
    if (focusShare() < 0.5) {
      note("La galería no parece centrada en «" + q.tag.replace(/_/g, " ") + "»: pulsa «Evaluar» si aun así la quieres.");
      return;
    }
    if (marca === state.marca && state.rec.length) return;
    run(true);
  }
  ...
  function buildPanel(panel) {
    panel.appendChild(
      util.el(
        "p",
        "r34g-hint",
        "El recomendador lee lo que ya trae la propia página (la puntuación de rule34 y las etiquetas de cada miniatura) y le pone una nota de 0 a 100 a cada post para decirte cuáles merecen la pena bajar. Es todo local: sin claves, sin cuota y funciona igual para cualquier usuario. Ojo: rule34.xxx no publica cuánta gente ha visto cada post, así que «lo popular» se mide con la puntuación y con los posts que tiene la etiqueta del personaje en todo el sitio."
      )
    );

    var como = ui.section(panel, "El recomendador", "Se enciende solo en las galerías de búsqueda (las de un post no tienen cuadrícula).");
    ui.toggle({ section: como, key: "scOn", title: "Mostrar el recomendador", note: "La barra encima de la galería y la nota en cada miniatura." });
    ui.toggle({ section: como, key: "scAuto", title: "Puntuar solo al abrir la galería", note: "Solo si la búsqueda es una etiqueta y la galería está centrada en ella. Si no, se queda esperando a que pulses «Evaluar»." });
    ui.toggle({ section: como, key: "scBadges", title: "Nota en cada miniatura", note: "Pasa el ratón por encima para ver el desglose." });

    var cuan = ui.section(panel, "Qu\u00e9 recomienda");
    ui.seg({
      section: cuan,
      key: "scMode",
      title: "Criterio",
      note: "\u00abLos mejores N\u00bb llena la lista aunque la galer\u00eda sea floja; \u00abnota m\u00ednima\u00bb solo ense\u00f1a lo que pasa el corte.",
      options: [
        { label: "Los mejores N", value: "top" },
        { label: "Nota m\u00ednima", value: "min" }
      ]
    });
    ui.number({ section: cuan, key: "scTop", title: "Cu\u00e1ntos", min: 1, max: 100, step: 1, suffix: "posts" });
    ui.number({ section: cuan, key: "scMin", title: "Nota m\u00ednima", min: 10, max: 100, step: 5, suffix: "de 100" });

    var pes = ui.section(panel, "Qu\u00e9 pesa");
    ui.number({
      section: pes,
      key: "scVideo",
      title: "V\u00eddeo",
      note: "Puntos extra para los v\u00eddeos (los que llevan la etiqueta video/webm o la marca de la web).",
      min: 0,
      max: 10,
      step: 1,
      suffix: "puntos"
    });
    ui.toggle({ section: pes, key: "scSeen", title: "Restar a los ya vistos", note: "Los que ya abriste en este navegador bajan puestos." });
    ui.toggle({ section: pes, key: "scHist", title: "Restar a los ya descargados", note: "Los que ya est\u00e1n en tu historial de descargas bajan puestos." });
    ui.toggle({
      section: pes,
      key: "scDup",
      title: "Juntar variantes",
      note: "Si dos posts son la misma escena en distinto formato o duraci\u00f3n, solo se recomienda el mejor y el otro queda marcado como variante."
    });

    ui.button({
      section: pes,
      title: "Puntúa ahora la cuadrícula de esta página",
      label: "Evaluar esta página",
      onClick: function () {
        var rec = run(true);
        if (rec) util.toast(rec.length + " posts recomendados de " + state.posts.length);
      }
    });
  }
  ...
  R.onChange(function (key) {
    if (String(key || "").indexOf("sc") !== 0) return;
    sync(false);
  });
  ...
  R.scoreApi = {
    ...
    clear: clearMarks,
    state: state,
    variants: function (posts) {
      return variantGroups(posts || state.posts);
    },
    queueRecommended: queueRecommended
  };
...
(function () {
  ...
  var NOTES = [
    ["0.9.0", [
      "Llega el Recomendador: encima de la galer\u00eda te dice qu\u00e9 posts de la b\u00fasqueda que tienes delante merecen la pena. Le pone una nota de 0 a 100 a cada miniatura (verde, amarilla o gris seg\u00fan lo buena que sea) y, con el rat\u00f3n por encima, ense\u00f1a el desglose.",
      "La nota mezcla lo que ya trae la propia p\u00e1gina: la puntuaci\u00f3n que ense\u00f1a rule34, lo bien etiquetado que est\u00e1 el post, si encaja con lo que buscabas, la resoluci\u00f3n, si es v\u00eddeo o lleva sonido y la duraci\u00f3n; y resta por reposts, por variantes de la misma escena y por lo que ya has visto o descargado. Como rule34.xxx no publica cu\u00e1ntas veces se ha visto un post, lo popular se mide con la puntuaci\u00f3n y con los posts que tiene en todo el sitio la etiqueta del personaje (o del artista).",
      "El panel del Recomendador (se abre al pulsar su t\u00edtulo) lista los recomendados en orden, con un bot\u00f3n Descargar en cada fila, y \u00abDescargar los recomendados\u00bb los mete todos en la cola de descargas de una vez. En Ajustes \u2192 Recomendador eliges el criterio (los mejores N o nota m\u00ednima), cu\u00e1ntos, el peso de los v\u00eddeos, y si restar a los ya vistos, restar a los ya descargados y juntar variantes.",
      "Solo punt\u00faa por su cuenta cuando la galer\u00eda es de una etiqueta y casi todos los posts la llevan; si es una b\u00fasqueda mezclada se queda esperando a que pulses \u00abEvaluar\u00bb, para no puntuar cuarenta posts que no son de lo que buscabas.",
      "Fuera el motor de IA viejo. Se ha quitado entero: el desplegable de proveedores, las cadenas de reserva (Perchance, Pollinations, Groq, OpenRouter, OpenAI, Mistral, Ollama y el personalizado) y el bot\u00f3n \u00abAnalizar con IA\u00bb del an\u00e1lisis de etiquetas. Ahora hay una sola IA, fija y sin desplegables: Google Gemini (el Flash m\u00e1s nuevo). Pega tu clave de Google AI Studio una vez en Ajustes \u2192 Etiquetas \u2192 Prompt con IA y se queda guardada solo en tu navegador; el bot\u00f3n \u00abProbar conexi\u00f3n\u00bb dice si va bien y, si falla, el aviso cuenta el motivo exacto.",
      "El an\u00e1lisis de etiquetas sigue siendo local y gratis para todo el mundo; la IA solo redacta el prompt en ingl\u00e9s de la pesta\u00f1a \u00abIA\u00bb del recuadro del prompt, como antes. Si no has puesto clave, esa pesta\u00f1a lo dice y el prompt local sigue estando ah\u00ed. En Ajustes \u2192 Etiquetas \u2192 Prompt con IA hay un bot\u00f3n para borrar los prompts de IA que se hayan quedado guardados.",
    ]],
    ["0.8.9", [
      "La pesta\u00f1a \u00abIA\u00bb del prompt ya no se cae sola a \u00abLocal\u00bb. Mientras el motor trabaja se queda encendida y parpadeando (con el aviso de los segundos justo debajo) y, al terminar, ense\u00f1a su texto; si el an\u00e1lisis se repite por detr\u00e1s (la API a\u00f1ade etiquetas, vuelves al post, reabres el panel) el prompt de la IA se recupera de la cach\u00e9 del post en vez de perderse.",
      "Volver a pulsar la pesta\u00f1a \u00abIA\u00bb pide un prompt nuevo. Antes solo serv\u00eda la respuesta guardada y no hab\u00eda forma de pedir otra sin tocar el navegador; ahora, si cambias de etiquetas y quieres otro texto, pulse otra vez.",
      "Las etiquetas redundantes ya se quedan fuera de verdad. La IA no puede reactivar las que el an\u00e1lisis local hab\u00eda descartado (bastaba con que su respuesta las listara entre las compartidas para que volvieran al resultado y al prompt) ni descartar la que el an\u00e1lisis local conserva, as\u00ed que se acabaron los pares contradictorios del tipo \u00abcum_inside -> cum_in_pussy\u00bb y \u00abcum_in_pussy -> cum_inside\u00bb a la vez.",
      "El grupo \u00abRedundantes\u00bb sale entero descartado (su nota ahora lo dice): si enciendes una a mano, tu decisi\u00f3n se respeta aunque vuelvas a analizar. Y la IA que redacta el prompt ya no recibe las etiquetas descartadas: se le mandan aparte, con la orden de no usarlas.",
      "El grupo \u00abRelacionados\u00bb (pares que se solapan, una m\u00e1s general que la otra) tiene un bot\u00f3n para quitar de golpe la general de cada pareja en vez de ir una por una. Se puede volver a encender cualquiera con un clic.",
      "La pesta\u00f1a \u00abArtista: primero / al final\u00bb ya no se apaga cada vez que la IA termina: era un descuido al repintar los botones del recuadro.",
      "Al volver a analizar no se pierden tus decisiones: las etiquetas que encendiste o apagaste a mano y el prompt de la IA se mantienen."
    ]],
    ["0.8.8", [
