...
(function () {
  ...
  var FLAGS_POR_DEFECTO = {
    ...
    escenaAuto: false  // 📍 apuntar la escena leída sin preguntar (si no, se propone)
  };
  ...
  // La cajita con lo que se ha leído y no se ha apuntado (cuando no está puesto «📍 apuntarla sola»).
  function cajaEscenaSug(s) {
    var sug = s && s.escenaSug;
    if (!sug || !sug.cambios || !sug.cambios.length) return null;
    var caja = el("div", { class: "em-esc-sug" });
    caja.appendChild(el("div", { class: "em-esc-sug-t", text: "📍 He leído otra escena" + (sug.de ? " (" + sug.de + ")" : "") + ":" }));
    sug.cambios.forEach(function (c) {
      caja.appendChild(el("div", { class: "em-esc-sug-c" }, [
        el("b", { text: ESC_ETIQUETA[c.campo] + ": " }),
        el("span", { text: "«" + c.valor + "»" })
      ]));
    });
    caja.appendChild(el("div", { class: "em-acts" }, [
      btn("✓ Apuntar", "em-btn em-btn-mini em-btn-main", function () {
        var f2 = ficha();
        if (!f2) return;
        var antes = escenaLinea(f2);
        sug.cambios.forEach(function (c) { ponerEscena(f2, c.campo, c.valor, "leído"); });
        f2.escenaSug = null;
        guardar();
        aviso("📍 Escena: " + (escenaLinea(f2) || "(vacía)") + (antes === escenaLinea(f2) ? " (ya estaba)" : ""), false);
        render();
      }, { title: "Apuntar lo que he leído" }),
      btn("✗ No", "em-btn em-btn-mini", function () {
        var f2 = ficha();
        if (!f2) return;
        f2.escenaSug = null;
        guardar();
        render();
      }, { title: "Seguimos como estábamos" })
    ]));
    return caja;
  }
  ...
  function escribirEn(n, texto) {
    n.focus();
    fijarValor(n, texto);
    try { n.dispatchEvent(new Event("keyup", { bubbles: true })); } catch (e) {}
  }
  // Escribir en un campo suyo (los de su app son de React): poner `.value` a pelo **no le vale**,
  // porque React lleva su propia cuenta de lo que hay dentro, ve que no ha cambiado nada y se queda
  // con lo de antes. El truco es usar el `set` del prototipo (ese salta su contador) y avisar con un
  // `input`, que es lo que él escucha. Sin esto, lo que escribimos no llega al mensaje que envía.
  function fijarValor(n, texto) {
    if (!n) return false;
    if (n.tagName === "TEXTAREA" || n.tagName === "INPUT") {
      var proto = n.tagName === "TEXTAREA" ? window.HTMLTextAreaElement.prototype : window.HTMLInputElement.prototype;
      var set = null;
      try { set = Object.getOwnPropertyDescriptor(proto, "value").set; } catch (e) {}
      try { if (set) set.call(n, texto); else n.value = texto; } catch (e) { n.value = texto; }
      try { if (n.setSelectionRange) n.setSelectionRange(texto.length, texto.length); } catch (e) {}
    } else {
      try {
        n.focus();
        ...
        sel.removeAllRanges();
        sel.addRange(rango);
        if (!document.execCommand("insertText", false, texto)) n.textContent = texto;
      } catch (e) {
        n.textContent = texto;
      }
    }
  ...
  // --- 📌 la orden que va pegada a tus mensajes ---------------------------------------------------
  // Su memoria del bot deja 300 caracteres y, además, el modelo solo mira los últimos mensajes: si
  // dónde está y en qué postura no van delante de sus narices, se lo inventa otra vez (y cambia la
  // escena a mitad de conversación). Esto pega tu orden **al final del mensaje que envías**, entre
  // paréntesis, así que él la lee como nota OOC y no cuenta para las barras (`analizar` da peso 0 a
  // lo que va dentro de `( … )`). Entra en el 1.º, el 5.º, el 9.º… con «cada 4».
  function ordenTexto(s) {
  ...
  function tocaLaOrden(s) {
    if (!s || !s.flags.orden) return false;
    if (!firmaDeOrden(s)) return false;
    if (s.ordenPoner) return true;
    var n = (Number(s.ordenN) || 0) + 1;
    return (n - 1) % cadenciaDeOrden(s) === 0;
  }
  // Se llama justo antes de que salga el mensaje (al pulsar Enter y al pulsar su botón de enviar), y
  // lleva la cuenta de los mensajes que mandas para saber si a este le toca.
  function ponerOrden(s) {
    if (!s) return false;
    var caja = cajaDeTexto();
    if (!caja) return false;
    var txt = valorDe(caja);
    if (!txt.trim()) return false;                        // no hay mensaje: no se cuenta
    if (Date.now() - state.ultimaOrden < 800) return false; // el teclado y el botón a la vez
    var firma = firmaDeOrden(s);
    if (firma && txt.indexOf(firma) !== -1) return false;  // ya la lleva
    var puesto = false;
    if (firma && tocaLaOrden(s)) {
      fijarValor(caja, txt.replace(/\s+$/, "") + "\n\n" + firma);
      state.ultimaOrden = Date.now();
      puesto = true;
    }
    s.ordenPoner = false;                                  // «poner ya» se gasta en este envío
    s.ordenN = (Number(s.ordenN) || 0) + 1;
    guardar();
    return puesto;
  }
  // Si al final el clic no era de enviar (o su app no manda con Enter), se quita la orden otra vez y
  // el mensaje no se cuenta: así el contador de «cada 4» sigue siendo el de mensajes de verdad.
  function vigilarOrden(caja, s, puesto) {
    setTimeout(function () {
      if (!caja || !caja.isConnected) return;
      if (valorDe(caja).trim() !== String(puesto).trim()) return;   // lo ha enviado o lo ha tocado él
      deshacerOrden(caja, s);
    }, 600);
  }
  function deshacerOrden(caja, s) {
    var firma = firmaDeOrden(s);
    if (!caja || !firma) return false;
    var t = valorDe(caja);
    var i = t.indexOf(firma);
    if (i === -1) return false;
    fijarValor(caja, t.slice(0, i).replace(/\s+$/, ""));
    s.ordenN = Math.max(0, (Number(s.ordenN) || 1) - 1);
    guardar();
    return true;
  }
  ...
  function alTeclear(e) {
    ...
    if (ponerOrden(s)) vigilarOrden(caja, s, valorDe(caja));   // 📌 la orden, antes de que salga
    if (!s.flags.medir) return;
    var txt = valorDe(caja);
    if (!txt.trim()) return;
    setTimeout(function () {
  ...
  // --- el retrato del bot, con el gesto puesto ----------------------------------------------------
  // Se usa SU retrato tal cual (no se genera nada): encima se le ponen capas —rubor, corazones,
  // brillo, gotas— según cómo vaya la partida. El retrato se busca solo: primero el que ya tengamos
  // apuntado, si no el mayor de los suyos que haya en pantalla, y si no se le pregunta a su API por
  // la ficha del personaje (que trae `thumbnailURL`). También vale pegar la URL a mano.
  function varon(n, nombre, valor) { try { n.style.setProperty(nombre, valor); } catch (e) {} }
  function urlDeImagen(src) {
    var u = String(src || "");
    if (!u) return "";
    if (u.indexOf("/_next/image") !== -1) {          // su web las pasa por el optimizador de Next
      try {
        var dentro = new URL(u, location.origin).searchParams.get("url");
        if (dentro) return dentro;
      } catch (e) {}
    }
    return u;
  }
  function valeDeRetrato(url) {
    if (!/^https?:\/\//.test(String(url || ""))) return false;
    if (!/image-cdn\.flowgpt\.com|\/generated_images\/|\/trans-images\/|uploads\.dev/i.test(url)) return false;
  ...
  function sacarRetrato(obj, out, hondo) {
    out = out || {};
    if (!obj || typeof obj !== "object" || hondo > 6) return out;
    if (Array.isArray(obj)) {
      obj.forEach(function (x) { sacarRetrato(x, out, hondo + 1); });
      return out;
    }
    Object.keys(obj).forEach(function (k) {
      var v = obj[k];
      if (typeof v === "string" && /^https?:\/\//.test(v) && /(thumbnail|cover|background|portrait)/i.test(k)) {
        if (!out[k]) out[k] = v;
      } else if (v && typeof v === "object") {
        sacarRetrato(v, out, hondo + 1);
      }
    });
    return out;
  }
  function retratoDeLaFicha(res) {
    var t = sacarRetrato(res);
    return t.thumbnailURL || t.thumbnailUrl || t.coverURL || t.coverUrl ||
      t.backgroundURL || t.backgroundUrl || t.portraitURL || "";
  }
  function guardarRetrato(url) {
    var s = ficha();
  ...
  // La pregunta a su API. Con enfriamiento por bot: si falla, no se le da la lata en cada pintada.
  function pedirRetrato(s, forzar) {
    if (!s || !s.promptId || state.pidiendoRetrato) return Promise.resolve("");
    var id = s.promptId;
    if (!forzar && state.retratoPedido[id] && Date.now() - state.retratoPedido[id] < 600000) return Promise.resolve("");
    state.retratoPedido[id] = Date.now();
    state.pidiendoRetrato = true;
    return EM.api.personaje(id)
      .then(function (res) {
        state.pidiendoRetrato = false;
        var url = retratoDeLaFicha(res);
        if (url && ficha() && ficha().promptId === id) {
          guardarRetrato(url);
          render();
        }
        return url || "";
      })
      .catch(function () { state.pidiendoRetrato = false; return ""; });
  }
  function buscarRetrato(decir) {
    var s = ficha();
    if (!s) return Promise.resolve("");
    var enPantalla = retratoDeLaPantalla();
    if (enPantalla) {
      guardarRetrato(enPantalla);
      render();
      if (decir) aviso("👤 Retrato encontrado en la pantalla.", false);
      return Promise.resolve(enPantalla);
    }
    if (decir) aviso("🔎 Buscándolo en su ficha…", false);
    return pedirRetrato(s, true).then(function (url) {
      if (decir) aviso(url ? "👤 Retrato encontrado." : "👤 No lo he encontrado: pégame su URL (la de image-cdn.flowgpt.com).", !url);
      render();
      return url;
    });
  }
  // Qué gesto le toca, según cómo va la partida (los valores van de 0 a 100).
  function animo(s) {
    var st = s.stats || {};
    var v = function (x) { return Number(x) || 0; };
    var muy = function (x) { return v(x) >= 65; };
    var algo = function (x) { return v(x) >= 40; };
    var d = v(st.deseo), t = v(st.tension), af = v(st.afecto), cf = v(st.confianza);
    if (muy(d) && muy(t)) return { id: "excitada", titulo: "Excitada", txt: "se le suben los colores y no disimula", rubor: 1, corazones: 5, brillo: 0.7, tiembla: true };
    if (muy(d)) return { id: "enamorada", titulo: "Enamorada", txt: "corazones por todas partes", rubor: 0.85, corazones: 4, brillo: 0.55, tiembla: false };
    if (muy(t)) return { id: "nerviosa", titulo: "Nerviosa", txt: "rubor flojo, gotas y un temblor", rubor: 0.32, corazones: 0, brillo: 0.2, tiembla: true, gota: true };
    if (muy(af) && algo(cf)) return { id: "cariñosa", titulo: "Cariñosa", txt: "brillo cálido y algún corazón", rubor: 0.5, corazones: 2, brillo: 0.85, tiembla: false };
    if (algo(t)) return { id: "tensa", titulo: "Tensa", txt: "un poco de rubor", rubor: 0.3, corazones: 0, brillo: 0.15, tiembla: false };
    if (algo(d)) return { id: "ilusionada", titulo: "Ilusionada", txt: "sonrojada y con un corazón suelto", rubor: 0.6, corazones: 1, brillo: 0.35, tiembla: false };
    return { id: "tranquila", titulo: "Tranquila", txt: "sin gesto: la partida va calmada", rubor: 0, corazones: 0, brillo: 0, tiembla: false };
  }
  // Las capas del gesto (brillo, rubor, corazones, gota). Se usan tal cual en la ficha y, estiradas
  // por `--em-esc`, encima de su retrato grande: por eso no llevan medidas fijas propias.
  function capasDeGesto(a) {
    var capas = [el("i", { class: "em-brillo", style: { opacity: String(a.brillo || 0) } })];
    if (a.rubor) {
      capas.push(el("i", { class: "em-rubor em-rubor-i", style: { opacity: String(a.rubor) } }));
      capas.push(el("i", { class: "em-rubor em-rubor-d", style: { opacity: String(a.rubor) } }));
    }
    for (var i = 0; i < a.corazones; i++) {
      var c = el("i", { class: "em-corazon", text: "❤" });
      varon(c, "--x", (10 + i * 18) + "%");
      varon(c, "--del", (i * 0.5) + "s");
      varon(c, "--tam", "calc(" + (9 + (i % 3) * 3) + "px * var(--em-esc, 1))");
      capas.push(c);
    }
    if (a.gota) capas.push(el("i", { class: "em-gota", text: "💧" }));
    return capas;
  }
  // El bloque del retrato tal como se pinta en la ficha.
  function retrato(s) {
    var caja = el("div", { class: "em-retrato" });
    var a = animo(s);
    var url = (s.retrato && /^https?:/i.test(s.retrato)) ? s.retrato : retratoDeLaPantalla();
    if (!url) {
      pedirRetrato(s, false);
      var in_ = el("input", { class: "em-input em-retrato-url", type: "text", placeholder: "…o pega su URL: https://image-cdn.flowgpt.com/…" });
      in_.value = state.retratoUrl || "";
      in_.addEventListener("input", function () { state.retratoUrl = in_.value; });
      caja.appendChild(el("div", { class: "em-cara em-cara-vacia", text: "👤" }));
      caja.appendChild(el("div", { class: "em-retrato-txt" }, [
        el("b", { text: "Su retrato" }),
        el("span", { class: "em-hint", text: "Todavía no sé cuál es. Búscalo o pégame su dirección y le pongo el gesto encima según la partida." }),
        el("div", { class: "em-acts" }, [
          el("button", { type: "button", class: "em-btn em-btn-mini em-btn-main", text: "🔎 Buscar", onclick: function () { buscarRetrato(true); } }),
          in_,
          el("button", { type: "button", class: "em-btn em-btn-mini", text: "Usar", onclick: function () {
            var u = (state.retratoUrl || "").trim();
            if (!/^https?:\/\//.test(u)) { aviso("Eso no parece una dirección de imagen.", true); render(); return; }
            if (!guardarRetrato(u)) return;
            state.retratoUrl = "";
            aviso("👤 Retrato guardado: ya se le pone el gesto encima.", false);
            render();
          } })
        ])
      ]));
    ...
    if (s.retrato !== url) { s.retrato = url; guardar(); }   // apuntado lo que se encontró en pantalla
    var ajuste = s.cara || (s.cara = { y: 42, x: 15 });
    var capas = [el("img", { class: "em-cara-img", src: url, alt: "Retrato del bot" })].concat(capasDeGesto(a));
    var cara = el("div", {
      class: "em-cara em-animo-" + a.id + (a.tiembla ? " em-tiembla" : ""),
      title: "Su retrato, con el gesto que le toca ahora (pulsa para verlo entero)"
    }, capas);
    varon(cara, "--em-cara-y", (Number(ajuste.y) || 42) + "%");
    varon(cara, "--em-cara-x", (Number(ajuste.x) || 15) + "%");
    cara.addEventListener("click", function () { try { window.open(url, "_blank"); } catch (e) {} });
    caja.appendChild(cara);
    caja.appendChild(el("div", { class: "em-retrato-txt" }, [
      el("b", { text: a.titulo }),
      el("span", { class: "em-hint", text: a.txt }),
      el("div", { class: "em-retrato-tune" }, [
        btn("⬆", "em-icon em-tune", function () { moverCara(-4, 0); }, { title: "Subir el rubor (si le cae en la boca)" }),
        btn("⬇", "em-icon em-tune", function () { moverCara(4, 0); }, { title: "Bajar el rubor (si le cae en los ojos)" }),
        btn("⬅", "em-icon em-tune", function () { moverCara(0, 3); }, { title: "Separar las mejillas (caras más anchas)" }),
        btn("➡", "em-icon em-tune", function () { moverCara(0, -3); }, { title: "Juntar las mejillas (caras más estrechas)" })
      ])
    ]));
    return caja;
  }
  // Mover el rubor hasta que caiga en las mejillas de SU retrato (queda apuntado por bot).
  function moverCara(dy, dx) {
    var s = ficha();
    if (!s) return;
    var c = s.cara || (s.cara = { y: 42, x: 15 });
    c.y = Math.max(20, Math.min(86, (Number(c.y) || 42) + dy));
    c.x = Math.max(6, Math.min(34, (Number(c.x) || 15) + dx));
    guardar();
    render();
  }

  // --- el mismo gesto, encima de su retrato GRANDE de la web (🖼) ---------------------------------
  // La ficha lo enseña en pequeño (64 px), pero el retrato de verdad es el que pinta su web. Aquí se
  // le pone una capa por encima, del tamaño que tenga en pantalla, con las mismas capas del ánimo. No
  ...
  function retratoNodoDeLaPantalla() {
    ...
    Array.prototype.forEach.call(imgs, function (n) {
      ...
      if (seVe(n) && a > areaVisible) { areaVisible = a; visible = n; }
    });
    return visible || otro;
  }
  function quitarDeLaWeb() {
    if (vivo.reloj) { clearInterval(vivo.reloj); vivo.reloj = null; }
    if (vivo.caja && vivo.caja.parentNode) vivo.caja.parentNode.removeChild(vivo.caja);
    vivo.caja = null;
    vivo.img = null;
    vivo.sello = "";
  }
  ...
  function escalaDeFoto(r) {
    return Math.max(1, Math.min(3, (r.width || 64) / 64));
  }
  function ponerSitio(r) {
    var c = vivo.caja;
    if (!c) return;
    c.style.setProperty("left", Math.round(r.left) + "px", "important");
    c.style.setProperty("top", Math.round(r.top) + "px", "important");
    c.style.setProperty("width", Math.round(r.width) + "px", "important");
    c.style.setProperty("height", Math.round(r.height) + "px", "important");
  }
  function pintarEnLaWeb() {
    var s = ficha();
    if (!s || !s.flags.retrato || !s.flags.retratoGrande) { quitarDeLaWeb(); return false; }
    var img = retratoNodoDeLaPantalla();
    if (!img) { quitarDeLaWeb(); return false; }
    var r = img.getBoundingClientRect();
    if (r.width < 8 || r.height < 8) { quitarDeLaWeb(); return false; }
    var a = animo(s);
    var ajuste = s.cara || (s.cara = { y: 42, x: 15 });
    var esc = escalaDeFoto(r);
    var sello = [a.id, a.rubor, a.brillo, a.corazones, a.gota ? 1 : 0, a.tiembla ? 1 : 0,
      ajuste.y, ajuste.x, esc.toFixed(2)].join("|");
    if (vivo.caja && vivo.img === img && vivo.sello === sello) { ponerSitio(r); return true; }
    if (!vivo.caja) {
      vivo.caja = el("div", { class: "em-live", "aria-hidden": "true" });
    ...
    var caja = vivo.caja;
    EM.clear(caja);
    caja.className = "em-live em-animo-" + a.id + (a.tiembla ? " em-tiembla" : "");
    capasDeGesto(a).forEach(function (n) { caja.appendChild(n); });
    varon(caja, "--em-cara-y", (Number(ajuste.y) || 42) + "%");
    varon(caja, "--em-cara-x", (Number(ajuste.x) || 15) + "%");
    varon(caja, "--em-esc", esc.toFixed(2));
    ponerSitio(r);
    return true;
  }
  // El latido: recolocar sobre la foto, y volver a pintarlo si su web ha cambiado la que se ve.
  function seguirEnLaWeb() {
    var s = ficha();
    if (!s || !s.flags.retrato || !s.flags.retratoGrande) { quitarDeLaWeb(); return false; }
  ...
  function vista() {
    ...
    // el retrato del bot, con el gesto que le toca ahora
    if (s.flags.retrato) cuerpo.appendChild(retrato(s));

    // 📍 lo que se ha leído de la escena y espera el visto bueno
    var sug = cajaEscenaSug(s);
    if (sug) cuerpo.appendChild(sug);
    ...
    // 📍 la escena: dónde estáis, en qué postura, con qué ropa y con qué luz
    var esc = plegable("📍 La escena" + (escenaLinea(s) ? " · " + recortaA(escenaLinea(s), 34) : ""), "escena");
    esc.appendChild(el("div", { class: "em-hint", text: "Los cuatro datos que el bot se inventa si no se los dices. Van en la nota de su memoria y, si quieres, pegados a tus mensajes (con 📌 va en la orden). Se pueden leer del chat solos: 🔍." }));
    ESC_CAMPOS.forEach(function (c) {
      var in_ = el("input", { class: "em-input em-esc-in", type: "text", placeholder: c.ph });
      in_.value = s.escena[c.id] || "";
      in_.addEventListener("input", function () {
        s.escena[c.id] = in_.value;
        guardar();
      });
      esc.appendChild(el("div", { class: "em-esc-row" }, [
        el("span", { class: "em-esc-lbl", text: c.label }),
        in_
      ]));
    });
    esc.appendChild(el("pre", { class: "em-pre", text: escenaLinea(s) || "(sin escena todavía)" }));
    esc.appendChild(el("div", { class: "em-acts" }, [
      btn("🔍 Mirar el chat", "em-btn em-btn-mini", function () { mirarEscenaDelChat(true); }, { title: "Lee los mensajes que hay en pantalla y busca sitio, postura, ropa y luz" }),
      btn((s.escenaEnOrden ? "✓ " : "· ") + "📌 va en la orden", "em-btn em-btn-mini em-toggle" + (s.escenaEnOrden ? " em-toggle-on" : ""), function () {
        var f2 = ficha();
        if (!f2) return;
        f2.escenaEnOrden = !f2.escenaEnOrden;
        guardar();
        render();
      }, { title: "La escena se pega delante de tu orden, en la línea que va con tus mensajes" }),
      btn("🧹 Vaciar", "em-btn em-btn-mini em-btn-bad", function () { olvidarEscena(true); })
    ]));
    // la tablita de palabras de la escena (el mismo formato que las de las barras, con un campo más)
    var etb = details("🔑 Palabras de la escena (" + escenaTabla().length + ")", state.escenaPalabras);
    etb.addEventListener("toggle", function () {
      if (state.escenaPalabras === etb.open) return;
      state.escenaPalabras = etb.open;
      render();
    });
    if (state.escenaPalabras) {
      var taE = el("textarea", { class: "em-input em-pal-edit", rows: 12, spellcheck: "false" });
      taE.value = state.edicionEscena != null ? state.edicionEscena : escenaTablaTexto();
      taE.addEventListener("input", function () { state.edicionEscena = taE.value; });
      etb.appendChild(el("div", { class: "em-hint", text: "Una línea por entrada:  nombre : palabras : campo : lo que se apunta ·  el campo es lugar / postura / ropa / ambiente, y los grupos van con |, las palabras de un grupo con + y los sinónimos con /." }));
      etb.appendChild(taE);
      etb.appendChild(el("div", { class: "em-acts" }, [
        btn("💾 Guardar", "em-btn em-btn-main em-btn-mini", function () {
          var t = state.edicionEscena != null ? state.edicionEscena : escenaTablaTexto();
          if (!deTextoEscena(t).length) {
            aviso("Eso no tiene ni una entrada que se pueda leer: revisa el formato (nombre : palabras : campo : lo que se apunta).", true);
            render();
            return;
          }
          setEscenaTablaTexto(t);
          state.edicionEscena = null;
          aviso("📍 Tabla de la escena guardada (" + escenaTabla().length + " entradas).", false);
          render();
        }),
        btn("↩ Tabla base", "em-btn em-btn-mini", function () {
          resetEscena();
          state.edicionEscena = null;
          aviso("📍 Vuelta a las palabras de serie.", false);
          render();
        }, { disabled: escenaEsBase() })
      ]));
      var salidaE = el("div", { class: "em-hint em-pal-out", text: "escribe una frase…" });
      var pruebaE = el("input", { class: "em-input", type: "text", placeholder: "Prueba: «*me siento a tu lado en el sofá*»" });
      pruebaE.addEventListener("input", function () {
        var r = escenaDe(pruebaE.value);
        salidaE.textContent = r.length
          ? r.map(function (c) { return ESC_ETIQUETA[c.campo] + " → «" + c.valor + "»"; }).join(" · ")
          : (String(pruebaE.value || "").trim() ? "nada (ni sitio, ni postura, ni ropa, ni luz)" : "escribe una frase…");
      });
      etb.appendChild(el("div", { class: "em-field" }, [pruebaE, salidaE]));
    }
    esc.appendChild(etb);
    ...
    // 📌 la orden que viaja pegada a tus mensajes
    var cada = cadenciaDeOrden(s);
    var ord = plegable("📌 Que no se pierda (" + (s.flags.orden ? (cada === 1 ? "cada mensaje" : "cada " + cada) : "apagado") + ")", "orden");
    ord.appendChild(el("div", { class: "em-hint", text: "El bot solo mira los últimos mensajes: si dónde está y en qué postura no van delante de sus narices, se los inventa (y te cambia la escena a mitad). Escribe aquí tu orden y se pega al final del mensaje que envías, entre paréntesis: él lo lee como nota y eso no cuenta para las barras." }));
    ...
    var pintarNumOrd = function () {
      ...
      numOrd.className = "em-count-num" + (t.length > LIMITE_ORDEN ? " em-count-on" : "");
    };
    ...
    pintarNumOrd();
    ord.appendChild(el("div", { class: "em-counted" }, [taOrd, numOrd]));
    ord.appendChild(el("pre", { class: "em-pre", text: firmaDeOrden(s) || "(todavía sin orden: escribe arriba)" }));
    ord.appendChild(el("div", { class: "em-acts" }, [1, 2, 4].map(function (n) {
      return btn((cada === n ? "✓ " : "· ") + (n === 1 ? "cada mensaje" : "cada " + n),
        "em-btn em-btn-mini em-toggle" + (cada === n ? " em-toggle-on" : ""), function () {
          var f2 = ficha();
          if (!f2) return;
          f2.ordenCada = n;
          guardar();
          render();
        }, { title: n === 1 ? "Va en todos los mensajes" : "Va en el 1.º, el " + (n + 1) + ".º, el " + (2 * n + 1) + ".º…" });
    })));
    ord.appendChild(el("div", { class: "em-hint", text: "Llevas " + (Number(s.ordenN) || 0) + " mensajes contados." }));
    ord.appendChild(el("div", { class: "em-acts" }, [
      btn("📌 Poner ya", "em-btn em-btn-mini em-btn-main", function () {
        var f2 = ficha();
        if (!f2) return;
        if (!firmaDeOrden(f2)) { aviso("Escribe primero tu orden.", true); render(); return; }
        f2.ordenPoner = true;
        guardar();
        aviso("📌 Va pegada al próximo mensaje que mandes.", false);
        render();
      }, { title: "Que entre en el próximo mensaje, toque o no según el turno" }),
      btn("↺ Volver a contar", "em-btn em-btn-mini", function () {
        var f2 = ficha();
        if (!f2) return;
        f2.ordenN = 0;
        guardar();
        aviso("↺ El próximo mensaje lleva la orden.", false);
        render();
      }, { title: "Empieza la cuenta otra vez: el siguiente mensaje la lleva" })
    ]));
    cuerpo.appendChild(ord);
    ...
    aj.appendChild(el("div", { class: "em-toggles" }, [
      ...
      interruptor("retrato", "👤 retrato con gesto", "Enseña su retrato con rubor, corazones y demás, según cómo vaya la partida"),
      interruptor("retratoGrande", "🖼 gesto en su foto grande", "Pinta ese mismo rubor y esos corazones encima de su retrato grande de la web, no solo en el pequeño de la ficha (el ajuste ⬆⬇⬅➡ vale para los dos)"),
      interruptor("orden", "📌 repetir mi orden", "Pega tu línea (en paréntesis) al final de los mensajes que envías, cada 1, 2 o 4, para que no se le olvide dónde está ni en qué postura"),
      interruptor("escenaAuto", "📍 apuntarla sola", "La escena que se lee del chat (sitio, postura, ropa, luz) se apunta sin preguntarte; si lo apagas, te la propone y decides tú")
    ]));
    aj.appendChild(el("div", { class: "em-acts" }, [
      btn("💗 A cero", "em-btn em-btn-mini em-btn-bad", function () {
        ...
        f.etapa = 0;
        f.turnos = 0;
        f.ordenN = 0;      // 📌 el primer mensaje de la partida nueva lleva la orden
        ESC_CAMPOS.forEach(function (c) { f.escena[c.id] = ""; });   // 📍 escena nueva
        f.escenaSug = null;
        f.vinculo = 0;
        f.log = [];
        f.vistos = [];
  ...
  function render() {
    state.sheet = ficha();
    pintarEnLaWeb();          // el gesto, también en su retrato grande (si está encendido)
    var d = state.dock;
    if (!d) return;
  ...
  EM.rpg = {
    ...
    retrato: retrato,
    animo: animo,
    retratoDeLaPantalla: retratoDeLaPantalla,
    retratoNodoDeLaPantalla: retratoNodoDeLaPantalla,
    pintarEnLaWeb: pintarEnLaWeb,
    seguirEnLaWeb: seguirEnLaWeb,
    quitarDeLaWeb: quitarDeLaWeb,
    retratoDeLaFicha: retratoDeLaFicha,
    buscarRetrato: buscarRetrato,
    moverCara: moverCara,
    ponerOrden: ponerOrden,
    tocaLaOrden: tocaLaOrden,
    firmaDeOrden: firmaDeOrden,
    ordenTexto: ordenTexto,
    cadenciaDeOrden: cadenciaDeOrden,
    deshacerOrden: deshacerOrden,
    escenaDe: escenaDe,
    escenaLinea: escenaLinea,
    ponerEscena: ponerEscena,
    sugerirEscena: sugerirEscena,
    mirarEscenaDelChat: mirarEscenaDelChat,
    mirarEscenaLista: mirarEscenaLista,
    olvidarEscena: olvidarEscena,
    escenaTabla: escenaTabla,
    escenaTablaTexto: escenaTablaTexto,
    setEscenaTablaTexto: setEscenaTablaTexto,
