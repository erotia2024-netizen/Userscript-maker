...
(function () {
  ...
  // solo con los juegos. Es el mismo anuncio, en el mismo hueco, cargado igual; se coloca con
  // `position:absolute` en vez de mover el nodo del DOM porque mover un iframe lo recargaría y
  // contaría otra impresión. Las medidas (dónde cae el cuadrado y cuánto hay que bajar la columna
  // para que no se pisen) se toman en cada pasada, así que aguanta que cambie la ventana o el
  // contenido.
  var adBase = 0;
  var adWritten = "";
  var COL_GAP = 16;
  ...
  function unsit() {
    ...
    if (col && col.dataset.hfSide) {
      ["position", "left", "top", "width", "z-index"].forEach(function (p) {
  ...
  function layoutAds() {
    ...
    var colW = col.offsetWidth || 300;
    var colH = col.offsetHeight || 254;
