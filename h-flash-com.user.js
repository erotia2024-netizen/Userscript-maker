...
(function () {
  ...
  function createEmbed() {
    ...
    if (f) {
      // El camino del propio sitio: cuenta la reproducción, monta el controlador y respeta lo que
      // el sitio tenga configurado. Es el que hay que usar siempre que se pueda.
      try {
        f();
        return true;
      } catch (e) {}
    }
    // Plan B (la web cambió y ya no hay `embedswf`): el `<embed>` de siempre, con lo que el sitio
    // trae en `swfpath` y sus medidas.
  ...
  function start() {
    ...
    if (playing() || q("#embedswf")) {
      // ya hay reproductor (el sitio lo montó porque el navegador dice tener Flash, o porque el
    ...
    if (ruffleReady()) {
      // Hay Ruffle cargado (extensión del navegador, o una carga anterior): solo hace falta el
  ...
  function stageHeight() {
    ...
    // Un poco menos de dos tercios de la ventana: el juego se ve grande pero sigue asomando el
    // título y la barra del reproductor por abajo.
  ...
  function fit() {
    if (!stage) return;
    var h = stageHeight();
    stage.style.setProperty("--hf-stage-h", h + "px");
    ...
    if (ctrl && ctrl.info && typeof ctrl.resize === "function") {
      ...
      ctrl.info.maxheight = aspect > 1 ? h : Math.min(h, Math.round(w / aspect));
      ctrl.info.maxwidth = aspect > 1 ? Math.min(w, Math.round(h * aspect)) : w;
      try {
        ctrl.resize("fit");
      } catch (e) {}
      return;
  ...
  function zoom(flag) {
    ...
    if (ctrl && typeof ctrl.resize === "function") {
      if (ctrl.info) {
        ctrl.info.maxwidth = box.clientWidth || box.offsetWidth || 970;
        ctrl.info.maxheight = stageHeight();
      }
      try {
        ctrl.resize(flag);
        return;
