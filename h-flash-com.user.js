...
(function () {
  ...
  function fit() {
    ...
    if (ctrl && ctrl.info) {
      ...
      ctrl.info.maxheight = h;
      try {
        ctrl.resize("fit");
      } catch (e) {}
    } else {
      // sin controlador: que el propio Ruffle llene el marco (escala la película y la centra)
      var e = q("#embedswf");
      if (e) {
        e.style.width = "100%";
        e.style.height = "100%";
  ...
  function mount() {
    box = q("#gamecontainer");
    if (!box || stage) return;
    stage = hf.el("div", { id: "hf-stage", cls: "hf-stage" });
    box.parentNode.insertBefore(stage, box);
